...
$pdo=db();
$checks=[
 'overlapping active appointments'=>"SELECT COUNT(*) FROM appointments a JOIN appointments b ON a.staff_id=b.staff_id AND a.appointment_date=b.appointment_date AND a.id<b.id AND a.start_time<b.end_time AND b.start_time<a.end_time WHERE a.status IN ('pending','confirmed') AND b.status IN ('pending','confirmed')",
