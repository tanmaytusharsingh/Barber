...
usort($selectedServices,fn($a,$b)=>(int)$a['id']<=>(int)$b['id']);
...
$staff=[];
if ($validSelection) {
    $staff=booking_staff_candidates($salonId,$serviceIds);
}
$splitPlan=[];
if ($validSelection && !$staff) {
    try { $splitPlan=booking_plan($salon,$serviceIds,0,is_array($_POST['staff_plan']??null)?$_POST['staff_plan']:[]); }
...
$planItems=[];
if ($validSelection && ($person || $splitPlan)) foreach ($selectedServices as $item) $planItems[]=['id'=>(int)$item['id'],'staff_id'=>$person?(int)$person['id']:$splitPlan[(int)$item['id']],'duration_minutes'=>(int)$item['duration_minutes']];
$slots=$planItems?available_plan_slots($salon,$planItems,$date):[];
...
<fieldset class="col-12"><legend class="h6">1. Services — choose one or more</legend><p class="small muted">Services run consecutively. Select your services, then click Show available times.</p><div class="row g-2"><?php foreach($services as $item): ?><div class="col-md-6"><label class="d-flex gap-2 border rounded p-3 h-100"><input class="form-check-input flex-shrink-0" type="checkbox" name="service_ids[]" value="<?= $item['id'] ?>" <?= in_array((int)$item['id'],$serviceIds,true)?'checked':'' ?>><span><?= e($item['name']) ?><small class="d-block muted"><?= money($item['price']) ?> · <?= (int)$item['duration_minutes'] ?> min</small></span></label></div><?php endforeach; ?></div></fieldset>
<?php if($staff): ?><div class="col-md-6"><label for="staff" class="form-label">2. Specialist for all services</label><select id="staff" name="staff_id" class="form-select"><option value="">Choose a specialist</option><?php foreach($staff as $item): ?><option value="<?= $item['id'] ?>" <?= (int)$item['id']===$staffId?'selected':'' ?>><?= e($item['name']) ?></option><?php endforeach; ?></select></div><?php elseif($splitPlan): ?><div class="col-12"><strong>2. Specialists assigned for your services</strong><p class="small muted mb-1">No one specialist performs every selected service. We assigned an available qualified specialist to each service.</p><?php foreach($selectedServices as $item): ?><?php $assigned=query('SELECT name FROM staff WHERE id=?',[$splitPlan[(int)$item['id']]])->fetchColumn(); ?><div><?= e($item['name']) ?> — <?= e($assigned) ?></div><?php endforeach; ?></div><?php endif; ?>
...
<input type="hidden" name="request_key" value="<?= e($requestKey) ?>"><input type="hidden" name="salon_id" value="<?= $salonId ?>"><?php foreach($serviceIds as $selectedId): ?><input type="hidden" name="service_ids[]" value="<?= $selectedId ?>"><?php if($splitPlan): ?><input type="hidden" name="staff_plan[<?= $selectedId ?>]" value="<?= $splitPlan[$selectedId] ?>"><?php endif; ?><?php endforeach; ?><input type="hidden" name="staff_id" value="<?= $staffId ?>"><input type="hidden" name="appointment_date" value="<?= e($date) ?>">
...
<script>document.querySelectorAll('input[name="service_ids[]"]').forEach(input=>input.addEventListener('change',()=>{const staff=document.getElementById('staff');if(staff)staff.value='';const booking=document.querySelector('form[method="post"]');if(booking)booking.hidden=true;}));document.getElementById('staff')?.addEventListener('change',function(){this.form.requestSubmit()});</script>
